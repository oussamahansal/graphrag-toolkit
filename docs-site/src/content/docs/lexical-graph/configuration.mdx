...
| `include_domain_labels` | Determines whether entities will have a domain-specific label (e.g. `Company`) as well as the [graph model's](/graphrag-toolkit/lexical-graph/graph-model/#entity-relationship-tier) `__Entity__` label | `False` | `INCLUDE_DOMAIN_LABELS` |
...
| `enable_versioning` | Whether to enable versioned updates (see [Versioned Updates](/graphrag-toolkit/lexical-graph/versioned-updates/)) | `False` | `ENABLE_VERSIONING` |
...
See [Using AWS Profiles in `GraphRAGConfig`](/graphrag-toolkit/lexical-graph/aws-profile/) for more details on configuring and using AWS named profiles.
